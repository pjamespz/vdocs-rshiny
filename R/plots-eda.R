...
#'
#' @description Creates a horizontal dot plot using custom ggplot theme, as an
#'   alternative to grouped bar charts.
#'
...
plot_horizontal_dotplot <- function(data, x_str, y_str, color_str = NULL,
                                    ...
                                    point_size = 8, theme_options = NULL,
                                    show_plot = FALSE, ...) {
  ...
  range_df <- data %>%
    ...
    dplyr::summarise(min = min(.data[[x_str]]),
                     max = max(.data[[x_str]])) %>%
    dplyr::ungroup() %>%
    dplyr::mutate(
      dplyr::across(
  ...
  if (is.null(theme_options)) {
    ...
    if (is.null(theme_options)) {
      ...
      for (opt in names(default_theme_options)) {
        if (!(opt %in% names(theme_options))) {
          theme_options[[opt]] <- default_theme_options[[opt]]
        }
      }
    }
    plt <- plt +
